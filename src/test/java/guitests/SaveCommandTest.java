...
//@@author A0140036X
/**
 * SaveCommandTest tests the saves command which saves current data
 * into newes
 */
...
public class SaveCommandTest extends TaskManagerGuiTest {
    ...
    public static class LoadUiState extends UiState {
        ...
        public LoadUiState(SaveCommandTest test) {
            super(test);
        }
        ...
        public void onResumeState() {
            ((SaveCommandTest) getGuiTest()).loadFromFilePath(savedLocation);
        }
    ...
    //@@author A0140036X
    /**
     * Tests loading of invalid file path that will throw an IOException in MainApp.java
     */
    @Test
    public void load_invalidPath() {
        LoadUiState state = new LoadUiState(this);
        state.saveState();
        assertLoad("", new ReadOnlyTask[0]);
        state.resumeState();
    }
